...
%% data valid signal
data_valid_in.time = [];
data_valid_in.signals.values = blank_sig;
data_valid_in.signals.values(valid_pps(1):end)=1;

% vtp valid signal
vtp_val_in.time = [];
vtp_val_in.signals.values = make_valid(frame_len,2,valid_pps(1),blank_sig);

vtp_in.time=[];
vtp_in.signals.values = blank_sig;

%% hdr in and hdr valid in
hdr_val_in.time = [];
hdr_val_in.signals.values=make_valid(frame_len,8,valid_pps(1),blank_sig);

hdr_in.time = [];
hdr_in.signals.values = blank_sig;

%% write words 0 and 1 in
w0_val = nth_valid(hdr_val_in.signals.values,1);
w1_val = nth_valid(hdr_val_in.signals.values,2);
...
secs_ref_ep_in.signals.values(w0_val~=1)=0;
...
%% make hdr data
hdr_in.signals.values = fnum_in.signals.values + secs_ref_ep_in.signals.values;
